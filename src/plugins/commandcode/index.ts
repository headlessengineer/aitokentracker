...
import { emptyPluginData, buildPluginData, globFiles, pathExists, convStatus, parseClaudeStyleJsonl,
  availResult,
} from '../core/collect'
import { sinceDate } from '../../lib/since'

const DATA_DIR = path.join(os.homedir(), '.commandcode', 'projects')

const COMMANDCODE_PLUGIN: TokenPlugin = {
  id: 'commandcode',
  name: 'Command Code',
  icon: 'Cd',
  description: 'Tracks token usage from Command Code AI sessions (~/.commandcode/projects)',
  dataPath: DATA_DIR,
  ...
  async collect(options?: CollectOptions): Promise<PluginData> {
    ...
    if (files.length === 0) return emptyPluginData('commandcode')
    ...
    return buildPluginData('commandcode', conversations, options)
  },
}

export default COMMANDCODE_PLUGIN
