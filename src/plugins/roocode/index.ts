...
const ROOCODE_PLUGIN: TokenPlugin = {
  id: 'roocode',
  name: 'Roo Code',
  icon: 'Rc',
  description: 'Tracks token usage from Roo Code VS Code extension tasks',
  dataPath: TASKS_DIR,
  ...
  async collect(options?: CollectOptions): Promise<PluginData> {
    ...
    let taskDirs: string[]
    try { taskDirs = await fs.readdir(TASKS_DIR) } catch { return emptyPluginData('roocode') }
    ...
    return buildPluginData('roocode', conversations, options)
  },
}

export default ROOCODE_PLUGIN
